...
type PbToGoItem struct {
	m            *proto.Message
	output       string //输出生成文件路径 ./Cx_output
	file_content string
	fileName     string
	Name         string
	EnumMap      []*EnumGoTableInfo
	ClassMap     []*ClassGoTableInfo
	packageName  string
}

func NewPbToGoItem() *PbToGoItem {
	return &PbToGoItem{}
}

// 打开协议文件
func (t *PbToGoItem) OpenProtoFile(fileName string, path string, output string) {
	t.fileName = fileName
	t.output = output
...
func (t *PbToGoItem) DoSheetTable(definition *proto.Proto) {
	// 遍历消息和字段
...
func (t *PbToGoItem) WLine(format string, a ...any) {
