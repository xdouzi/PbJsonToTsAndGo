...
type PbToJavaItem struct {
	m            *proto.Message
	output       string //输出生成文件路径 ./Cx_output
	file_content string
	fileName     string
	Name         string
	EnumMap      []*EnumJavaTableInfo
	ClassMap     []*ClassJavaTableInfo
	packageName  string
}

func NewPbToJavaItem() *PbToJavaItem {
	return &PbToJavaItem{}
}

// 打开协议文件
func (t *PbToJavaItem) OpenProtoFile(fileName string, path string, output string) {
	t.fileName = fileName
	t.output = output
...
func (t *PbToJavaItem) DoSheetTable(definition *proto.Proto) {
	// 遍历消息和字段
	proto.Walk(definition,
		...
		proto.WithEnum(func(p *proto.Enum) {
			//处理美剧
			e := NewEnumJavaTableInfo(p)
			t.EnumMap = append(t.EnumMap, e)
		}),
		proto.WithMessage(func(m *proto.Message) {
			t.m = m
			q := NewClassJavaTableInfo()
			q.DoData(m)
...
func (t *PbToJavaItem) SaveFile() {
	...
	outputFile := fmt.Sprintf("%s/%s.java", t.output, t.Name)
	// 将配置文件写入文件中
...
func (t *PbToJavaItem) WLine(format string, a ...any) {
